...
import { saveToMoviesInWishList } from "./saved-movies-local-storage.js";
...
async function generateMoviesData() {
  ...
  myData = myData.filter((item) => item.type === "Movie");

  dataMovies(myData);
  my_data_movies = myData;
  searchMovies();
  saveButtonClicked();
}
...
let app = document.querySelector(".products-movies-list");
...
function dataMovies(data) {
  ...
  app.addEventListener("click", (event) => {
    ...
    if (activeDiv) {
      ...
      saveToMoviesInWishList(my_data_movies, index);
    }
  });
}
